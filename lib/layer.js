const debug = require('debug')('traceview:layer')
const Event = require('./event')
const util = require('./util')
const tv = require('./')
const addon = tv.addon
...
Layer.prototype.init = function (name, parent, data) {
  ...
  let context = addon.Context.toString()

  // Construct blank entry and exit events.
  let entry = new Event(name, 'entry', parent && context)
  let exit = new Event(name, 'exit', entry.event)
...
Layer.prototype.descend = function (name, data) {
  ...
  let layer = new Layer(name, Event.last, data)
...
Layer.prototype.profile = function (name, data) {
  ...
  let profile = new Profile(name, Event.last, data)
...
Layer.prototype.runAsync = function (fn) {
  ...
  layer.enter()
  let ret = fn.call(layer, function (cb, handler) {
...
Layer.prototype._internal = function (label, data) {
  ...
  let event = new Event(null, label, last)
