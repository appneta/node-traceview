 ...
 * @type String
 */
Object.defineProperty(exports, 'appToken', {
  get () {
    return appToken
  },
  set (value) {
...
//
if (!accessKey) {
  const cuuid = process.env.TRACEVIEW_CUUID
  if (cuuid) {
    exports.accessKey = cuuid
  } else {
    // Attempt to find access_key in tracelyzer configs
    const configFile = '/etc/tracelytics.conf'
...
if (!enabled) {
  ...
  exports.sample = function (layer, xtrace, meta, url) {
    ...
    if (!rv.length && !exports.skipSample) {
      return false
    }
  ...
  exports.startOrContinueTrace = function (xtrace, build, run, opts, cb) {
    ...
    if (xtrace && typeof xtrace === 'object') {
      xid = xtrace.xid
      meta = xtrace.meta
      url = xtrace.url || xtrace.identity
    }

    // Do sampling
    let sampled
    try {
      sampled = exports.sample(data.name, xid, meta, url)
    } catch (e) {
    ...
    try {
      ...
      if (exports.always && !xid) {
        exports.addEntryData(layer.events.entry)
      }
    } catch (e) {
      error('tv.startOrContinueTrace failed to build layer', e.stack)
    }
  ...
  /*!
   * Expose an interface to add sample data to an event.
   * This is needed to allow http probes access to to this.
   */
  exports.addEntryData = function (event) {
    // If appToken has been explicitly set, include AApp k/v pair
    if (appToken) {
      event.set({ AApp: appToken })
    }

    // Include App and _SP data
    event.set({
      App: bindings.Context.defaultAppToken,
      _SP
    })
  }
  ...
  exports.initData = function () {
    ...
    // Include App and possibly AApp data
    data.App = bindings.Context.defaultAppToken
    if (appToken) {
      data.AApp = appToken
    }
  ...
  process.nextTick(function () {
    exports.requestStore.run(function () {
      const layer = new Layer('nodejs', null, exports.initData())
      layer.enter()
      layer.exit()
    })
  })
